...
class Posting(models.Model):
    ...
    image = ProcessedImageField(
        ...
        processors=[ResizeToFit(width=960, upscale=False)],  # upscale 의 의미: 작은거를 늘릴건지
        format='JPEG'
    )

    # thumbnail
    image_thumbnail = ImageSpecField(
        source='image',  # 위에서 만든 resize 된 이미지 변수명
    ...
    def save(self, *args, **kwargs):
        ...
        if self.image:
            print(f'    imaze_size: {self.image.width}px * {self.image.height}px: {round(self.image.size / 1024)}kb')
        print('=========================================')
        print()
