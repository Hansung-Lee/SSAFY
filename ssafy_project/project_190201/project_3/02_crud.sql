-- 1. 최근 영화 극한직업이 누락되어 있습니다.
-- 영진위 API에서 얻은 정보에 따르면, 위와 같습니다.
...
-- 2. 데이터 수집과정에서 실수로 과거의 데이터가 포함되었습니다.
-- 영화코드가 20040521인 데이터를 출력하세요. 그리고, 해당 데이터를 삭제하세요.
SELECT * FROM movies WHERE 영화코드 = 20040521;
DELETE FROM movies WHERE 영화코드 = 20040521;


-- 3. 영화코드 20185124인 데이터를 출력하세요.
